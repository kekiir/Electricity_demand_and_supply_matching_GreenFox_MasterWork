...
public class TimeServiceImp implements TimeService {
  ...
  public void validateGivenDates(String fromString, String toString) throws IllegalArgumentException,
    ...
    if (now.isAfter(twoHourBeforeTodayMidnight) && now.isBefore(todayMidnight))
    ...
    validateDateRange(from, to, afterTomorrowMidnight);
  ...
  public void validateDateRange(LocalDateTime from, LocalDateTime to, LocalDateTime afterTomorrowMidnight) {
    ...
    if (!from.isAfter(afterTomorrowMidnight) || !to.isAfter(afterTomorrowMidnight)) {
